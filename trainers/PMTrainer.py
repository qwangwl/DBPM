...
class PMTrainer(BaseTrainer):
    ...
    def train(self, source_loaders, target_loader):
        ...
        for epoch in range(self.n_epochs):
            ...
            log.append([loss_clf, loss_transfer, loss_pll, source_acc, target_acc])
    ...
    def test2(self, dataloader):
        feature = dataloader.dataset.data()
        labels = dataloader.dataset.label()
        cluster = dataloader.dataset.cluster()
        labels = np.argmax(labels.numpy(), axis=1)

        y_preds = self.model.predict2(feature.to(self.device), cluster).cpu().numpy()        
        acc = np.sum(y_preds == labels) / len(labels)
        return acc * 100.

    def post_epoch_processing(self, source_loader,  target_loader, epoch):
        tgt_x = target_loader.dataset.data()
        tgt_cluster = target_loader.dataset.cluster()

        src_x = source_loader.dataset.data()
        src_y = source_loader.dataset.label()
        src_cluster = source_loader.dataset.cluster()
